(function () {
      ...
      (v.fn = v.prototype =
        ...
        v.extend({
          noConflict: function (t) {
            return (
              e.$ === v && (e.$ = a), t && e.jQuery === v && (e.jQuery = u), v
            );
          },
          isReady: !1,
          readyWait: 1,
        ...
        val: function (e) {
          ...
          if (!arguments.length) {
            if (s)
              return (
                (n =
                  v.valHooks[s.type] || v.valHooks[s.nodeName.toLowerCase()]),
          ...
          if (!s || (s.nodeType !== 3 && s.nodeType !== 8)) {
            ...
            for (f = 0; f < m.length && !n.isPropagationStopped(); f++)
              (l = m[f][0]),
                (n.type = m[f][1]),
                (d =
                  (v._data(l, "events") || {})[n.type] && v._data(l, "handle")),
                d && d.apply(l, r),
                (d = h && l[h]),
                d &&
                  v.acceptData(l) &&
          ...
          (v.event.special.submit = {
            setup: function () {
              ...
              v.event.add(this, "click._submit keypress._submit", function (e) {
                var n = e.target,
                  r =
                    v.nodeName(n, "input") || v.nodeName(n, "button")
                      ? n.form
                      : t;
                r &&
            ...
            return v.access(
              ...
              function (e) {
                return e === t
                  ? v.text(this)
                  : this.empty().append(
                      ((this[0] && this[0].ownerDocument) || i).createTextNode(
                        e
                      )
                    );
              },
  ...
  define("jqueryPlugins", ["jquery"], function ($) {
          ...
          { borderSpacing: -150, opacity: 1 },
          {
            step: function (now, fx) {
              if (fx.prop == "borderSpacing") {
                $(this)
                  .css("-webkit-transform", "rotate(" + now + "deg)")
    ...
    if (
      $.browser.mobile &&
      window.matchMedia &&
      !window.matchMedia("screen and (max-screen-width: 480px)").matches
    ) {
  ...
  */
    ...
    */
      ...
      util.assert(
        url.search(/^(http:\/\/|https:\/\/|\/\/|data:)/i) === 0,
        "URL must have an http, https, data, or // scheme:",
        url
      );
      util.assert(
        url.search(/[\)\'\"\ ]/) === -1,
        "URLs cannot contain ), ', \", or spaces:",
        JSON.stringify(url)
      );
    };
    ...
    util.readFileImage = function (el) {
      return util.Deferred(function (def) {
        ...
        reader.onload = function () {
          def.resolve(
            "data:image/jpeg;base64," + util.blobToBase64(this.result)
          );
        };
...
*/
      ...
      _setupConnection: function () {
        ...
        this.socket.onclose = function (event) {
          ...
          if (!this.closed) {
            ...
            var time = Math.min(
              this._backoff * this.backoffTime,
              this.maxBackoffTime
            );
            setTimeout(
              function () {
    ...
    channels.PostMessageChannel = util.Class(AbstractChannel, {
      ...
      _setupConnection: function () {
        ...
        var time =
          this._pingPollPeriod + this._pingPollIncrease * this._pingFailures;
  ...
  define("storage", ["util"], function (util) {
    ...
    storage.settings = util.mixinEvents({
      ...
      get: function (name) {
        assert(
          storage.settings.defaults.hasOwnProperty(name),
          "Unknown setting:",
          name
        );
  ...
  define("session", [
    ...
    "jquery",
    "storage",
  ], function (require, util, channels, $, storage) {
    var DEBUG = true;
    ...
    session.hubUrl = function (id) {
      ...
      assert(
        id,
        "URL cannot be resolved before TogetherJS.shareId has been initialized"
      );
    ...
    session.shareUrl = function () {
      assert(
        session.shareId,
        "Attempted to access shareUrl() before shareId is set"
      );
    ...
    session.recordUrl = function () {
      ...
      var url =
        TogetherJS.baseUrl.replace(/\/*$/, "") + "/togetherjs/recorder.html";
      url +=
        "#&togetherjs=" +
        session.shareId +
        "&hubBase=" +
        TogetherJS.config.get("hubBase");
      return url;
    };
    ...
    function openChannel() {
      ...
      c.onmessage = function (msg) {
        ...
        if (!peers) {
          ...
          console.warn(
            "Message received before all modules loaded (ignoring):",
            msg
          );
          return;
        }
        if (
          !msg.clientId &&
          MESSAGES_WITHOUT_CLIENTID.indexOf(msg.type) == -1
        ) {
          console.warn(
            "Got message without clientId, where clientId is required",
            msg
          );
          return;
        }
    ...
    function getRoomName(prefix, maxSize) {
      var findRoom =
        TogetherJS.config.get("hubBase").replace(/\/*$/, "") + "/findroom";
      return $.ajax({
        url: findRoom,
        dataType: "json",
      ...
      return util.Deferred(function (def) {
        ...
        if (!shareId) {
          ...
          if (m) {
            ...
            shareId = m[2];
            var newHash =
              hash.substr(0, m.index) + hash.substr(m.index + m[0].length);
        ...
        return storage.tab.get("status").then(function (saved) {
          ...
          if (findRoom && saved && findRoom != saved.shareId) {
            ...
            console.info(
              "Ignoring findRoom in lieu of explicit invite to session"
            );
          }
          if (
            findRoom &&
            typeof findRoom == "string" &&
            !saved &&
            ...
            assert(
              findRoom.prefix && typeof findRoom.prefix == "string",
              "Bad findRoom.prefix",
              findRoom
            );
            assert(
              findRoom.max &&
                typeof findRoom.max == "number" &&
                findRoom.max > 0,
              "Bad findRoom.max",
              findRoom
            );
            ...
            if (findRoom.prefix.search(/[^a-zA-Z0-9]/) != -1) {
              console.warn(
                "Bad value for findRoom.prefix:",
                JSON.stringify(findRoom.prefix)
              );
            }
            ...
            throw new util.AssertionError(
              "No saved status, and no startup._launch request; why did TogetherJS start?"
            );
          }
  ...
  define("peers", ["util", "session", "storage", "require"], function (
    util,
    session,
    storage,
    require
      ...
      peers.Self = util.mixinEvents({
        ...
        _loadFromSettings: function () {
          return util
            ...
            .then(
              function (name, avatar, defaultName, color) {
                if (!defaultName) {
                ...
                if (!avatar) {
                  avatar =
                    TogetherJS.baseUrl +
                    "/togetherjs/images/default-avatar.png";
                }
                this.update({
                  name: name,
                  avatar: avatar,
        ...
        _loadFromApp: function () {
          ...
          if (getUserName) {
            ...
            if (name && typeof name != "string") {
              ...
              console.warn(
                "Error in getUserName(): should return a string (got",
                name,
                ")"
              );
              name = null;
          ...
          if (getUserColor) {
            ...
            if (color && typeof color != "string") {
              ...
              console.warn(
                "Error in getUserColor(): should return a string (got",
                color,
                ")"
              );
              color = null;
          ...
          if (getUserAvatar) {
            ...
            if (avatar && typeof avatar != "string") {
              console.warn(
                "Error in getUserAvatar(): should return a string (got",
                avatar,
                ")"
              );
              avatar = null;
    ...
    TogetherJS.config.track(
      ...
      TogetherJS.config.track(
        "getUserColor",
        TogetherJS.config.track("getUserAvatar", function () {
          if (peers.Self) {
            peers.Self._loadFromApp();
          }
        })
   ...
   * License, v. 2.0. If a copy of the MPL was not distributed with this file,
   * You can obtain one at http://mozilla.org/MPL/2.0/. */
  define("windowing", ["jquery", "util", "peers", "session"], function (
    $,
    util,
    peers,
    session
    ...
    windowing.hide = function (els) {
      ...
      els =
        els ||
        ".togetherjs-window, .togetherjs-modal, .togetherjs-notification";
      els = $(els);
      ...
      els.each(function (index, element) {
        ...
        bound = $(bound);
        bound
          .addClass("togetherjs-animated")
          .addClass("togetherjs-color-pulse");
        setTimeout(function () {
          bound
            .removeClass("togetherjs-color-pulse")
            .removeClass("togetherjs-animated");
  ...
  define("templating", [
    ...
    "windowing",
    "session",
  ], function ($, util, peers, windowing, session) {
    var assert = util.assert;
    ...
    templating.sub = function (templateId, variables) {
      ...
      util.forEachAttr(variables, function (value, attr) {
        ...
        var subs = template
          .find(".togetherjs-sub-" + attr)
          .removeClass("togetherjs-sub-" + attr);
        if (subs.length) {
        ...
        var ifs = template
          .find(".togetherjs-if-" + attr)
          .removeClass("togetherjs-sub-" + attr);
        if (!value) {
          ifs.hide();
        }
        ifs = template
          .find(".togetherjs-ifnot-" + attr)
          .removeClass("togetherjs-ifnot-" + attr);
        if (value) {
          ifs.hide();
        }
        ...
        texts.forEach(function (item) {
          if (item.nodeType == document.ELEMENT_NODE) {
            ...
            while (true) {
              ...
              var leadingNode = document.createTextNode(
  ...
  define("elementFinder", ["util", "jquery"], function (util, $) {
    ...
    elementFinder.findElement = function findElement(loc, container) {
      ...
      if (loc === "body") {
        ...
        if (!el) {
          throw elementFinder.CannotFind(
            "#" + id,
            "No element by that id",
            container
          );
        }
        if (rest) {
        ...
        if (!el) {
          throw elementFinder.CannotFind(
            ":nth-child(" + num + ")",
            "container only has " + (num - count) + " elements",
            container
          );
        }
        if (loc) {
  ...
  define("ui", [
    ...
    function panelPosition() {
      ...
      if (iface.hasClass("togetherjs-dock-right")) {
        ...
        throw new AssertionError(
          "#togetherjs-dock doesn't have positioning class"
        );
      }
    }
    ...
    ui.prepareUI = function () {
      ...
      ui.container
        .find(".togetherjs-window > header, .togetherjs-modal > header")
        .each(function () {
    ...
    ui.activateUI = function () {
      if (deferringPrepareUI) {
        console.warn(
          "ui.activateUI called before document is ready; waiting..."
        );
      ...
      container
        .find("input.togetherjs-share-link")
      ...
      input.on("input propertychange", function () {
        ...
        $("#togetherjs-chat-input-box").height(
          $("#togetherjs-chat-input-box").height() + diff
        );
        $("#togetherjs-chat-messages").height(
          $("#togetherjs-chat-messages").height() - diff
        );
        return false;
      });
      ...
      function openDock() {
        ...
        $(
          "#togetherjs-dock-anchor #togetherjs-dock-anchor-horizontal img"
        ).attr("src", src);
      }
      ...
      $("#togetherjs-menu-feedback, #togetherjs-menu-feedback-button").click(
        function () {
          windowing.hide();
          hideMenu();
      ...
      $("#togetherjs-menu-help, #togetherjs-menu-help-button").click(
        function () {
          windowing.hide();
          hideMenu();
      ...
      $("#togetherjs-menu .togetherjs-self-name").bind(
        "keyup change",
        function (event) {
      ...
      $(
        "#togetherjs-menu-update-avatar, #togetherjs-menu-update-avatar-button"
      ).click(function () {
        hideMenu();
      ...
      $("#togetherjs-menu-update-color").click(function () {
        ...
        bindPicker();
        picker
          .find(".togetherjs-swatch-active")
          .removeClass("togetherjs-swatch-active");
        picker
          .find('.togetherjs-swatch[data-color="' + peers.Self.color + '"]')
          .addClass("togetherjs-swatch-active");
      });
    ...
    function adjustDockSize(buttons) {
      ...
      assert(
        newHeight >= BUTTON_HEIGHT * 3,
        "Height went too low (",
        newHeight,
        "), should never be less than 3 buttons high (",
        BUTTON_HEIGHT * 3,
        ")"
      );
      iface.css({
    ...
    ui.chat = {
      text: function (attrs) {
        ...
        var lastEl = ui.container.find(
          "#togetherjs-chat .togetherjs-chat-message"
        );
        if (lastEl.length) {
        ...
        if (
          lastEl &&
          lastEl.attr("data-person") == attrs.peer.id &&
          lastDate &&
          date < lastDate + COLLAPSE_MESSAGE_LIMIT
        ) {
    ...
    ui.PeerView = util.Class({
      constructor: function (peer) {
        assert(
          peer.isSelf !== undefined,
          "PeerView instantiated with non-Peer object"
        );
        this.peer = peer;
      ...
      setElement: function (el) {
        ...
        var classes = [
          "togetherjs-person",
          "togetherjs-person-status",
          "togetherjs-person-name",
          "togetherjs-person-name-abbrev",
          "togetherjs-person-bgcolor",
          "togetherjs-person-swatch",
          "togetherjs-person-status",
          "togetherjs-person-role",
          "togetherjs-person-url",
          "togetherjs-person-url-title",
      ...
      updateDisplay: deferForContainer(function (container) {
        ...
        container
          .find("." + this.peer.className("togetherjs-person-name-"))
          .text(this.peer.name || "");
        container
          .find("." + this.peer.className("togetherjs-person-name-abbrev-"))
          .text(abbrev);
        var avatarEl = container.find(
          "." + this.peer.className("togetherjs-person-")
        );
        if (this.peer.avatar) {
        ...
        if (this.peer.color) {
          var colors = container.find(
            "." + this.peer.className("togetherjs-person-bgcolor-")
          );
          colors.css({
            backgroundColor: this.peer.color,
          });
          colors = container.find(
            "." + this.peer.className("togetherjs-person-bordercolor-")
          );
          colors.css({
            borderColor: this.peer.color,
          });
        ...
        container
          .find("." + this.peer.className("togetherjs-person-role-"))
        ...
        container
          .find("." + this.peer.className("togetherjs-person-url-title-"))
          .text(urlName);
        ...
        container
          .find("." + this.peer.className("togetherjs-person-url-"))
          .attr("href", url);
        // FIXME: should have richer status:
        container
          .find("." + this.peer.className("togetherjs-person-status-"))
        ...
        if (this.peer.isSelf) {
          ...
          if (!this.peer.name) {
            $("#togetherjs-menu .togetherjs-person-name-self").text(
              this.peer.defaultName
            );
          }
        }
        if (this.peer.url != session.currentUrl()) {
          container
            .find("." + this.peer.className("togetherjs-person-"))
      ...
      dock: deferForContainer(function () {
        ...
        function CollapsedDock() {
          ...
          $("#togetherjs-dock-participants").replaceWith(
            "<button id='togetherjs-participantlist-button' class='togetherjs-button'><div class='togetherjs-tooltip togetherjs-dock-person-tooltip'><span class='togetherjs-person-name'>Participants</span><span class='togetherjs-person-tooltip-arrow-r'></span></div><div class='togetherjs-person togetherjs-person-status-overlay' title='Participant List' style='background-image: url(http://localhost:8888/togetherjs/images/robot-avatar.png); border-color: rgb(255, 0, 0);'></div></button>"
          );
        ...
        this.dockElement.attr(
          "id",
          this.peer.className("togetherjs-dock-element-")
        );
        ui.container
          .find("#togetherjs-dock-participants")
          .append(this.dockElement);
        ...
        this.detailElement
          .find('[for="togetherjs-person-status-follow"]')
          .attr("for", followId);
        this.detailElement
          .find("#togetherjs-person-status-follow")
          .attr("id", followId);
      ...
      maybeHideDetailWindow: function (windows) {
        if (
          this.detailElement &&
          windows[0] &&
          windows[0][0] === this.detailElement[0]
        ) {
    ...
    function refreshInvite() {
      ...
      require(["who"], function (who) {
        ...
        function refresh(users, finished) {
          ...
          var pos = 0;
          ui.container
            .find("#togetherjs-invite-users .togetherjs-menu-item")
            .each(function () {
              var $this = $(this);
              ...
              while (
                pos < sorted.length &&
                $this.attr("data-clientid") !== sorted[pos].id
              ) {
  ...
  define("playback", [
    ...
    "storage",
    "require",
  ], function ($, util, session, storage, require) {
    ...
    playback.getLogs = function (url) {
      if (url.search(/^local:/) === 0) {
        return $.Deferred(function (def) {
          storage.get("recording." + url.substr("local:".length)).then(
            function (logs) {
              if (!logs) {
                def.resolve(null);
                return;
  ...
  define("chat", [
    ...
    var commands = {
      ...
      command_savelogs: function (name) {
        ...
        function save(msg) {
          if (
            msg.request.forClient == session.clientId &&
            msg.request.saveAs == name
          ) {
      ...
      command_baseurl: function (url) {
        ...
        storage
          ...
          .then(function () {
            ui.chat.system({
              text:
                "baseUrl overridden (to " + url + "), will last for one day.",
            });
          });
      },
    ...
    var Console = util.Class({
      ...
      setLevel: function (l) {
        ...
        if (typeof l == "string") {
          ...
          if (number === undefined) {
            throw new Error(
              "Tried to set Console level to unknown level string: " + l
            );
          }
          l = number;
        }
        if (typeof l == "function") {
          ...
          if (number == -1) {
            throw new Error(
              "Tried to set Console level based on unknown console function: " +
                l
            );
          }
          l = number;
        ...
        if (typeof l == "number") {
          if (l < 0) {
            ...
            throw new Error(
              "Console level must be " + this.maxLevel + " or smaller: " + l
            );
          }
        }
        this.level = l;
      ...
      suppressedWrite: function () {
        this.write.apply(
          this,
          ["suppress"].concat(Array.prototype.slice.call(arguments))
        );
      },
      ...
      _browserInfo: function () {
        ...
        return [
          ...
          "Age: " +
            this._formatMinutes(Date.now() - TogetherJS.pageLoaded) +
            " minutes",
      ...
      toString: function () {
        try {
          ...
          this.messages.forEach(function (m) {
            lines.push(
              lpad(this._formatTime(m[0]), 6) +
                " " +
                rpad(this._formatLevel(m[1]), 6) +
                " " +
                lpadLines(m[2], 14)
            );
          }, this);
      ...
      submit: function (options) {
        ...
        return util.Deferred(function (def) {
          ...
          var site =
            options.site ||
            TogetherJS.config.get("pasteSite") ||
  ...
  define("cursor", [
    ...
    var Cursor = util.Class({
      ...
      updatePeer: function (peer) {
        ...
        if (peer.status != "live") {
          ...
          this.element
            .find(".togetherjs-cursor-container")
            .animate(
              {
                width: 34,
                height: 20,
      ...
      setKeydown: function () {
        if (this.keydownTimeout) {
          ...
          this.element
            .find(".togetherjs-cursor-typing")
            .show()
            .animateKeyboard();
        }
        this.keydownTimeout = setTimeout(
          this.clearKeydown,
          this.KEYDOWN_WAIT_TIME
        );
      },
    ...
    function mousemove(event) {
      ...
      var parent = $(target).closest(
        ".togetherjs-window, .togetherjs-popup, #togetherjs-dock"
      );
      if (parent.length) {
        target = parent[0];
    ...
    function makeCursor(color) {
      ...
      context.lineTo(
        (Math.sin(CURSOR_ANGLE / 2) * CURSOR_HEIGHT) / 1.5,
        (Math.cos(CURSOR_ANGLE / 2) * CURSOR_HEIGHT) / 1.5
      );
      context.lineTo(
        (Math.sin(CURSOR_ANGLE) * CURSOR_HEIGHT) / 1.2,
    ...
    function documentClick(event) {
      ...
      setTimeout(function () {
        ...
        if (
          !util.matchElement(element, cloneClicks) &&
          util.matchElement(element, dontShowClicks)
        ) {
          return;
        }
  ...
  define("ot", ["util"], function (util) {
    ...
    var Change = util.Class({
      constructor: function (version, clientId, delta, known, outOfOrder) {
        ...
        assert(
          typeof version == "number" && typeof clientId == "string",
          "Bad Change():",
          version,
          clientId
        );
      ...
      isBefore: function (otherChange) {
        assert(
          otherChange !== this,
          "Tried to compare a change to itself",
          this
        );
        return (
          otherChange.version > this.version ||
          (otherChange.version == this.version &&
            otherChange.clientId > this.clientId)
        );
      },
      ...
      knowsAboutAll: function (versions) {
        for (var clientId in versions) {
          ...
          if (
            !this.known[clientId] ||
            this.known[clientId] < versions[clientId]
          ) {
            return false;
          }
        }
      ...
      maybeMissingChanges: function (mostRecentVersion, clientId) {
        ...
        if (
          (clientId > this.clientId &&
            this.known[clientId] >= this.version - 1) ||
    ...
    ot.SimpleHistory = util.Class({
      ...
      getSelection: function () {
        ...
        return [
          ...
          this.selection.start + this.selection.del,
        ];
      },
    ...
    ot.History = util.Class({
      ...
      add: function (change) {
        ...
        assert(
          ...
          "Got a change",
          change,
          "that appears older (or same as) a known change",
          this.known[change.clientId]
        );
        ...
        if (
          (last.clientId == "init" || last.isBefore(change)) &&
        ...
        if (
          change.maybeMissingChanges(this.mostRecentLocalChange, this.clientId)
        ) {
        ...
        if (!clientsToCheck.isEmpty()) {
          ...
          this._history.walkBack(function (c, index) {
            ...
            if (
              clientsToCheck.contains(c.clientId) &&
      ...
      logHistory: function (prefix) {
        ...
        console.log.apply(
          console,
          [prefix + "history", this.clientId, ":"].concat(postfix)
        );
      ...
      logChange: function (prefix, change, callback) {
        ...
        console.log.apply(
          console,
          [prefix, this.clientId, ":", change + ""]
            .concat(postfix)
            .concat([JSON.stringify(this.getStateSafe(true))])
        );
        try {
          callback();
        } finally {
          console.log(
            prefix + " after:",
            change + "",
            JSON.stringify(this.getStateSafe())
          );
        }
      },
      ...
      addDelta: function (delta) {
        ...
        var change = Change(
          ...
          this.clientId,
          delta,
          util.extend(this.knownVersions)
        );
        this.add(change);
        return change;
      },
    ...
    ot.TextReplace = util.Class({
      ...
      toString: function () {
        ...
        if (!this.del) {
          return (
            "[insert " + JSON.stringify(this.text) + " @" + this.start + "]"
          );
        } else if (!this.text) {
      ...
      apply: function (text) {
        ...
        return (
          text.substr(0, this.start) +
          this.text +
          text.substr(this.start + this.del)
        );
      },
      ...
      */
        var overlap;
        assert(
          delta instanceof ot.TextReplace,
          "Transposing with non-TextReplace:",
          delta
        );
        if (this.empty()) {
        ...
        if (delta.before(this)) {
          ...
          return [
            this.clone(this.start + delta.text.length - delta.del),
            delta.clone(),
          ];
          ...
          return [
            this.clone(),
            delta.clone(delta.start + this.text.length - this.del),
          ];
        } else if (delta.sameRange(this)) {
          //console.log("  =same range");
          return [
            this.clone(this.start + delta.text.length, 0),
            delta.clone(undefined, 0),
          ];
        } else if (delta.contains(this)) {
          //console.log("  =other contains this");
          return [
            this.clone(delta.start + delta.text.length, 0, this.text),
            delta.clone(
              undefined,
              delta.del - this.del + this.text.length,
          ...
          return [
            this.clone(
              undefined,
              this.del - delta.del + delta.text.length,
              delta.text + this.text
            ),
          ...
          return [
            this.clone(undefined, this.del - overlap),
            delta.clone(this.start + this.text.length, delta.del - overlap),
          ];
        } else {
          //console.log("  =this overlaps end of other");
          assert(
            delta.overlapsStart(this),
            delta + "",
            "does not overlap start of",
            this + "",
            delta.before(this)
          );
          overlap = delta.start + delta.del - this.start;
          return [
            this.clone(delta.start + delta.text.length, this.del - overlap),
      ...
      contains: function (other) {
        return (
          other.start >= this.start &&
          other.start + other.del < this.start + this.del
        );
      },
      ...
      classMethods: {
        ...
        fromChange: function (oldValue, newValue) {
          ...
          var removed = oldValue.substr(
            commonStart,
            oldValue.length - commonStart - commonEnd
          );
          var inserted = newValue.substr(
            commonStart,
            newValue.length - commonStart - commonEnd
          );
  ...
  define("forms", [
    ...
    "templating",
    "ot",
  ], function ($, util, session, elementFinder, eventMaker, templating, ot) {
    ...
    function sendData(attrs) {
      ...
      if (
        elementFinder.ignoreElement(el) ||
      ...
      if (isText(el) || tracker) {
        ...
        if (history) {
          ...
          el.data(
            "togetherjsHistory",
            ot.SimpleHistory(session.clientId, value, 1)
          );
        }
      } else {
        msg.value = value;
    ...
    TogetherJS.addTracker = function (TrackerClass, skipSetInit) {
      ...
      assert(
        typeof TrackerClass.prototype.trackerName === "string",
      ...
      "destroy update init makeInit tracked".split(/ /).forEach(function (m) {
        assert(
          typeof TrackerClass.prototype[m] === "function",
      ...
      "scan tracked".split(/ /).forEach(function (m) {
        assert(
          typeof TrackerClass[m] === "function",
    ...
    CKEditor.scan = function () {
      ...
      for (var instanceIdentifier in CKEDITOR.instances) {
        editorInstance =
          document.getElementById(instanceIdentifier) ||
          document.getElementsByName(instanceIdentifier)[0];
        if (editorInstance) {
    ...
    CKEditor.tracked = function (el) {
      ...
      el = $(el)[0];
      return !!(
        CKEDITOR.dom.element.get(el) && CKEDITOR.dom.element.get(el).getEditor()
      );
    };
      ...
      util.forEachAttr(editTrackers, function (TrackerClass) {
        ...
        if (els) {
          $.each(els, function () {
            ...
            $(this).data(
              "togetherjsHistory",
              ot.SimpleHistory(session.clientId, tracker.getContent(), 1)
            );
    ...
    function getTracker(el, name) {
      ...
      for (var i = 0; i < liveTrackers.length; i++) {
        ...
        if (tracker.tracked(el)) {
          assert(
            !name || name == tracker.trackerName,
            "Expected to map to a tracker type",
            name,
            "but got",
            tracker.trackerName
          );
    ...
    var TEXT_TYPES = (
      "color date datetime datetime-local email " + "tel text time week"
    ).split(/ /g);
    ...
    session.hub.on("form-update", function (msg) {
      ...
      if (msg.replace) {
        ...
        msg.replace.delta = ot.TextReplace(
          msg.replace.delta.start,
    ...
    function sendInit() {
      ...
      els.each(function () {
        if (
          elementFinder.ignoreElement(this) ||
          elementTracked(this) ||
          suppressSync(this)
        ) {
          return;
        ...
        try {
          ...
          if (update.basis) {
            ...
            if (
              ...
              $(el).data(
                "togetherjsHistory",
                ot.SimpleHistory(session.clientId, update.value, update.basis)
              );
            }
          }
        } finally {
      ...
      if (lastFocus) {
        setTimeout(function () {
          if (lastFocus) {
            session.send({
              type: "form-focus",
              element: elementFinder.elementLocation(lastFocus),
            });
          }
        });
      }
...
*/
  define("startup", [
    ...
    "windowing",
    "storage",
  ], function (util, require, $, windowing, storage) {
    var assert = util.assert;
  ...
  define("videos", ["jquery", "util", "session", "elementFinder"], function (
    $,
    util,
    session,
    elementFinder
  ...
  define("walkthrough", [
    ...
    "session",
    "peers",
  ], function (util, ui, $, windowing, templates, templating, session, peers) {
    var assert = util.assert;
    ...
    walkthrough.start = function (firstTime, doneCallback) {
      ...
      assert(
        typeof firstTime == "boolean",
        "You must provide a firstTime boolean parameter"
      );
      if (firstTime) {
    ...
    function show(index) {
      slides.hide();
      $(slides[index]).show();
      var bullets = container.find(
        "#togetherjs-walkthrough-progress .togetherjs-walkthrough-slide-progress"
      );
      bullets.removeClass("togetherjs-active");
      $(bullets[index]).addClass("togetherjs-active");
      var $next = $("#togetherjs-walkthrough-next").removeClass(
        "togetherjs-disabled"
      );
      var $previous = $("#togetherjs-walkthrough-previous").removeClass(
        "togetherjs-disabled"
      );
  ...
  define("webrtc", [
    ...
    "storage",
    "windowing",
  ], function (require, $, util, session, ui, peers, storage, windowing) {
    var webrtc = util.Module("webrtc");
    var assert = util.assert;

    session.RTCSupported = !!(
    ...
    if (
      session.RTCSupported &&
      $.browser.mozilla &&
      parseInt($.browser.version, 10) <= 19
    ) {
    ...
    function makePeerConnection() {
      ...
      throw new util.AssertionError(
        "Called makePeerConnection() without supported connection"
      );
    }
    ...
    function audioButton(selector) {
      ...
      if (selector == "#togetherjs-audio-incoming") {
        $("#togetherjs-audio-button")
          .addClass("togetherjs-animated")
          .addClass("togetherjs-color-alert");
      } else {
        $("#togetherjs-audio-button")
          .removeClass("togetherjs-animated")
          .removeClass("togetherjs-color-alert");
      }
    }
   ...
   * License, v. 2.0. If a copy of the MPL was not distributed with this file,
   * You can obtain one at http://mozilla.org/MPL/2.0/. */
  define("who", ["util", "channels", "session", "ui"], function (
    util,
    channels,
    session,
    ui
    ...
    function prepareYouTube() {
      ...
      window.onYouTubeIframeAPIReady = function () {
        ...
        $(youTubeIframes).each(function (i, iframe) {
          var player = new YT.Player(iframe.id, {
            // get the reference to the already existing iframe
            events: {
      ...
      function setupYouTubeIframes() {
        ...
        iframes.each(function (i, iframe) {
          ...
          if (
            ($(iframe).attr("src") || "").indexOf("youtube") != -1 &&
            !$(iframe).attr("id")
          ) {
